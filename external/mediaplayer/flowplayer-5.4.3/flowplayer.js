...
$j(function() {
   if (typeof $j.fn.flowplayer == 'function') {
      $j("video").parent(".flowplayer").flowplayer();
   }
});
...
window.flowplayer = function(fn) {
   return $j.isFunction(fn) ? extensions.push(fn) :
      typeof fn == 'number' || fn === undefined ? instances[fn || 0] :
      $j(fn).data("flowplayer");
};

$j(window).on('beforeunload', function() {
   $j.each(instances, function(i, api) {
      if (api.conf.splash) {
         api.unload();
      } else {
         api.bind("error", function () {
            $j(".flowplayer.is-error .fp-message").remove();
         });
      }
   });
});
...
// jQuery plugin
$j.fn.flowplayer = function(opts, callback) {

   if (typeof opts == 'string') opts = { swf: opts }
   if ($j.isFunction(opts)) { callback = opts; opts = {} }

   return !opts && this.data("flowplayer") || this.each(function() {

      // private variables
      var root = $j(this).addClass("is-loading"),
         conf = $j.extend({}, flowplayer.defaults, flowplayer.conf, opts, root.data()),
         videoTag = $j("video", root).addClass("fp-engine").removeAttr("controls"),
      ...
      if (conf.playlist.length) { // Create initial video tag if called without
         ...
         if (videoTag.length) videoTag.replaceWith(placeHolder = $j('<p />'));
         videoTag = $j('<video />').addClass('fp-engine');
         ...
         if (typeof conf.playlist[0] === 'string') videoTag.attr('src', conf.playlist[0]);
            ...
            $j.each(conf.playlist[0], function(i, plObj) {
               for (var type in plObj) {
                  if (plObj.hasOwnProperty(type)) {
                     videoTag.append($j('<source />').attr({type: 'video/' + type, src: plObj[type]}));
                  }
               }
            });
         }
      ...
      var api = oldApi || {
         ...
         load: function(video, callback) {
            ...
            $j.extend(video, engine.pick(video.sources));
            ...
            if (video.src) {
               ...
               if (!e.isDefaultPrevented()) {
                  ...
                  // callback
                  if ($j.isFunction(video)) callback = video;
         ...
         */
         seek: function(time, callback) {
            if (api.ready) {
               ...
               var ev = $j.Event('beforeseek');
               root.trigger(ev, [api, time]);
               if (!ev.isDefaultPrevented()) {
                  engine.seek(time);
                  if ($j.isFunction(callback)) root.one("seek", callback);
               } else {
                  api.seeking = false;
         ...
         speed: function(val, callback) {
            ...
            if (api.ready) {
               ...
               if (typeof val == "boolean") {
                  val = conf.speeds[$j.inArray(api.currentSpeed, conf.speeds) + (val ? 1 : -1)] || api.currentSpeed;
               }
      ...
      api.conf = $j.extend(api.conf, conf);


      /* event binding / unbinding */
      $j.each(['bind', 'one', 'unbind'], function(i, key) {
      ...
      if (!root.data('flowplayer')) { // Only bind once
         root.bind("boot", function() {
            ...
            // conf
            $j.each(['autoplay', 'loop', 'preload', 'poster'], function(i, key) {
            ...
            // 2. failed -> try another
               ...
               $j.each(flowplayer.engine, function(name, impl) {
                  if (name != conf.engine) {
            ...
            // unload others
            if (conf.splash) {
               $j(".flowplayer").filter(".is-ready, .is-loading").not(root).each(function() {
                  var api = $j(this).data("flowplayer");
...
!function() {
   ...
   var s = flowplayer.support,
      browser = $j.browser,
      video = $j("<video loop autoplay preload/>")[0],
      IS_IE = browser.msie,
   ...
   s.animation = (function() {
      var vendors = ['','Webkit','Moz','O','ms','Khtml'], el = $j("<p/>")[0];
...
function embed(swf, flashvars) {
   ...
   tag += $j.browser.msie ? 'classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000">' :
   ...
   // parameters
   $j.each(opts, function(key, value) {
   ...
   var engine = {
      ...
      pick: function(sources) {
         ...
         if (flowplayer.support.flashVideo) {
            ...
            var flash = $j.grep(sources, function(source) { return source.type == 'flash'; })[0];
      ...
      load: function(video) {
         ...
         // convert to absolute
         if (!is_absolute && !conf.rtmp) url = $j("<img/>").attr("src", url)[0].src;
         ...
         if (api) {
            ...
            // optional conf
            $j.each(['key', 'autoplay', 'preload', 'rtmp', 'loop', 'debug', 'preload', 'splash'], function(i, key) {
            ...
            // listen
            $j[callbackId] = function(type, arg) {

               if (conf.debug && type != "status") console.log("--", type, arg);

               var event = $j.Event(type);

               switch (type) {

                  // RTMP sends a lot of finish events in vain
                  // case "finish": if (conf.rtmp) return;
                  case "ready": arg = $j.extend(video, arg); break;
      ...
      unload: function() {
         ...
         delete $[callbackId];
         $j("object", root).remove();
         api = 0;
      }

   };

   $j.each("pause,resume,seek,volume".split(","), function(i, name) {
...
function findFromSourcesByType(sources, type) {
   var arr = $j.grep(sources, function(s) {
      return s.type === type;
   });
...
flowplayer.engine.html5 = function(player, root) {
   ...
   var videoTag = $j("video", root),
      support = flowplayer.support,
      track = $j("track", videoTag),
      conf = player.conf,
      self,
      timer,
      api;
   ...
   return self = {
      ...
      load: function(video) {
         ...
         listen(api, $j("source", videoTag).add(videoTag), video);
   ...
   function listen(api, sources, video) {
      ...
      sources.bind("error", function(e) {
         try {
            if (e.originalEvent && $j(e.originalEvent.originalTarget).is('img')) return e.preventDefault();
            if (canPlay($j(e.target).attr("type"))) {
      ...
      $j.each(EVENTS, function(type, flow) {
         ...
         api.addEventListener(type, function(e) {
            ...
            if (!player.ready && !/ready|error/.test(flow) || !flow || !$j("video", root).length) { return; }

            var event = $j.Event(flow), arg;
...
function URLResolver(videoTag) {
   ...
   // initial sources
   $j("source", videoTag).each(function() {
      sources.push(parseSource($j(this)));
   });
   ...
   self.resolve = function(video) {
      ...
      if ($j.isArray(video)) {
         ...
         video = { sources: $j.map(video, function(el) {
            var type, ret = $j.extend({}, el);
            $j.each(el, function(key, value) { type = key; });
            ret.type = type;
            ret.src = el[type];
            delete ret[type];
            return ret;
         ...
         $j.each(sources, function(i, source) {
   ...
   return this.each(function() {
      ...
      root.data("api", api).bind("mousedown.sld touchstart", function(e) {
         ...
         if (!disabled) {
            ...
            var delayedFire = $j.throttle(fire, 100);
            calc();
            api.dragging = true;
...
flowplayer(function(api, root) {
   ...
   function setRatio(val) {
      ...
      if (!support.inlineBlock) $j("object", root).height(root.height());
   }
   ...
   // misc
      ...
      if (error) {
         ...
         var el = $j(".fp-message", root);
         $j("h2", el).text((api.engine || 'html5') + ": " + error.message);
         $j("p", el).text(error.url || api.video.url || api.video.src || conf.errorUrls[error.code]);
   ...
   // click
      ...
      if ($j(e.target).is(".fp-ui, .fp-engine") || e.flash) {
         e.preventDefault();
         return api.toggle();
      }
   });
   ...
   $j(".fp-toggle, .fp-play", root).click(api.toggle);

   /* controlbar elements */
   $j.each(['mute', 'fullscreen', 'unload'], function(i, key) {
   ...
   find("time").click(function(e) {
      $j(this).toggleClass("is-inverted");
   });
...
$j(document).bind("keydown.fp", function(e) {
   ...
   if ($j.inArray(key, [63, 187, 191, 219]) != -1) {
...
flowplayer(function(api, root) {
   ...
   if (api.conf.tooltip) {
      $j(".fp-ui", root).attr("title", "Hit ? for help").on("mouseout.tip", function() {
         $j(this).removeAttr("title").off("mouseout.tip");
      });
   }

   $j(".fp-close", root).click(function() {
      root.toggleClass(IS_HELP);
   });

});

var VENDOR = $j.browser.mozilla ? "moz": "webkit",
   FS_ENTER = "fullscreen",
...
// esc button
$j(document).bind(FS_NATIVE_SUPPORT ? "fullscreenchange" : VENDOR + "fullscreenchange", function(e) {
   var el = $j(document.webkitCurrentFullScreenElement || document.mozFullScreenElement || document.fullscreenElement || e.target);
...
flowplayer(function(player, root) {
   ...
   var conf = $j.extend({ active: 'is-active', advance: true, query: ".fp-playlist a" }, player.conf),
      klass = conf.active;

   // getters
   function els() {
      return $j(conf.query, root);
   }

   function active() {
      return $j(conf.query + "." + klass, root);
   }
   ...
   player.play = function(i) {
      ...
      player.load(typeof player.conf.playlist[i] === 'string' ?
         ...
         $j.map(player.conf.playlist[i], function(item) { return $j.extend({}, item); })
      );
      return player;
   };
   ...
   $j('.fp-next', root).click(player.next);
   $j('.fp-prev', root).click(player.prev);
   ...
   if (player.conf.playlist.length) { // playlist configured by javascript, generate playlist
      ...
      if (!plEl.length) {
         plEl = $j('<div class="fp-playlist"></div>');
         var cntrls = $j('.fp-next,.fp-prev', root).eq(0).before(plEl);
         if (!cntrls.length) $j('video', root).after(plEl);
      }
      plEl.empty();
      $j.each(player.conf.playlist, function(i, item) {
         var href;
   ...
   if (els().length) {
      if (!playlistInitialized) {
         ...
         els().each(function() {
            ...
            $j(this).attr('data-index', player.conf.playlist.length);
      ...
      root.on("click", conf.query, function(e) {
         e.preventDefault();
         var el = $j(e.target).closest(conf.query);
      ...
      player.bind("load", function(e, api, video) {
         var prev = active().removeClass(klass),
            ...
            el = $j('a[data-index="' + index + '"]', root).addClass(klass),
...
flowplayer(function(player, root) {
   ...
   if (player.conf.generate_cuepoints) {
      ...
      player.bind("load", function() {
         ...
         $j(".fp-cuepoint", root).remove();

      }).bind("ready", function() {

         var cues = player.cuepoints || [],
            duration = player.video.duration,
            timeline = $j(".fp-timeline", root).css("overflow", "visible");

         $j.each(cues, function(i, cue) {

            var time = cue.time || cue;
            if (time < 0) time = duration + cue;

            var el = $j("<a/>").addClass("fp-cuepoint fp-cuepoint" + i)
...
flowplayer(function(player, root, engine) {
   ...
   $j.get(url, function(txt) {
      ...
      for (var i = 0, lines = txt.split("\n"), len = lines.length, entry = {}, title, timecode, text, cue; i < len; i++) {
         ...
         if (timecode) {
            ...
            // text
            text = "<p>" + lines[++i] + "</p><br/>";
            while ($j.trim(lines[++i]) && i < lines.length) text +=  "<p>" + lines[i] + "</p><br/>";
   ...
   var wrap = $j("<div class='fp-subtitle'/>", root).appendTo(root),
      currentPoint;
   ...
   player.bind("cuepoint", function(e, api, cue) {
      ...
      $j.each(player.cuepoints || [], function(i, cue) {
...
flowplayer(function(player, root) {
   ...
   if (id) {
      ...
      if (typeof _gat == 'undefined') $j.getScript("//google-analytics.com/ga.js");
...
if (flowplayer.support.touch || isIeMobile) {
   ...
   flowplayer(function(player, root) {
      ...
      // native fullscreen
      if (player.conf.native_fullscreen && ($j.browser.webkit || $j.browser.safari)) {
         player.fullscreen = function() {
            var video = $j('video', root);
      ...
      (isAndroid || isSilk) && player.bind("ready", function() {
         ...
         player.bind("progress.dur", function() {
            ...
            if (duration !== 1) {
               ...
               $j(".fp-duration", root).html(format(duration));
...
flowplayer(function(player, root) {
   ...
   var conf = player.conf,
      ui = $j(".fp-ui", root),
      trigger = $j("<a/>", { "class": "fp-embed", title: 'Copy to your site'}).appendTo(ui),
      target = $j("<div/>", { 'class': 'fp-embed-code'})
         .append("<label>Paste this HTML code on your site to embed.</label><textarea/>").appendTo(ui),
      area = $j("textarea", target);
   ...
   player.embedCode = function() {
      ...
      var video = player.video,
         ...
         el = $j("<div/>", { 'class': 'flowplayer', css: { width: width, height: height }}),
         tag = $j("<video/>").appendTo(el);

      // configuration
      $j.each(['origin', 'analytics', 'logo', 'key', 'rtmp'], function(i, key) {
         if (conf[key]) el.attr("data-" + key, conf[key]);
      });

      // sources
      $j.each(video.sources, function(i, src) {
         tag.append($j("<source/>", { type: "video/" + src.type, src: src.src }));
      });

      var scriptAttrs = { src: "//embed.flowplayer.org/5.4.3/embed.min.js" };
      if ($j.isPlainObject(conf.embed)) {
      ...
      var code = $j("<foo/>", scriptAttrs).append(el);
      return $j("<p/>").append(code).html().replace(/<(\/?)foo/g, "<$1script");
   };
...
$j.fn.fptip = function(trigger, active) {
   ...
   return this.each(function() {
      ...
      function close() {
         ...
         $j(document).unbind(".st");
      }

      $j(trigger || "a", this).click(function(e) {
         ...
         if (root.hasClass(active)) {
            ...
            $j(document).bind("keydown.st", function(e) {
               if (e.which == 27) close();

            // click:close
            }).bind("click.st", function(e) {
               if (!$j(e.target).parents("." + active).length) close();
            });
         }
